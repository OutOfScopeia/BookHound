...
## Workflow tips
* One way to work with this project is by cloning the repo inside the devcontainer, with the VS Code command **Dev Containers: Clone Repository in Container Volume**. That way,the project will build in ~50 seconds. This takes ~180 seconds or more if you instead clone the repo into a local folder which then gets synced into the container.
Working with the repo inside the container only you have to be mindful not to lose your work - if you screw up the container build scripts, the container won't build and you will lose access to it. Don't panic even if you delete the container, the volumes are maintained separately by docker and if you mount the right one you can salvage your unpushed code. In other words, you can cowboy with the containers, just don't be cowboy with the volumes.
* When working with a local copy, I have created for myself a VS Code key shortcut (Shift + Alt + Q) that toggles between **Dev Containers: Reopen in Container** and **Dev Containers: Reopen Folder Locally**.
